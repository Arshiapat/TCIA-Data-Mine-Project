...
class TCIADatasetAnalyzer:
    ...
    def _catalog_dicom_comprehensive(self, root_path: Path) -> Dict[str, Any]:
        ...
        # Filter out "Unknown" patients for accurate counting
        valid_patients = {pid: studies for pid, studies in patients.items() 
                         if pid not in ('Unknown', 'UNKNOWN', '') and pid.strip()}
        unknown_patient_count = len(patients) - len(valid_patients)
        
        total_patients = len(valid_patients)
        total_studies = sum(len(studies) for studies in valid_patients.values())
        total_series = sum(
            len(series) 
            for studies in valid_patients.values() 
        ...
        # Log warning if unknown patients were found
        if unknown_patient_count > 0:
            self._log(f"⚠️  Warning: {unknown_patient_count} DICOM file(s) with missing/invalid PatientID excluded from patient count")
        
        # Patient hierarchy summary (only valid patients)
        patient_summary = []
        for patient_id, studies in list(valid_patients.items())[:100]:  # First 100 patients
            patient_info = {
                "patient_id": patient_id,
        ...
        results = {
            ...
            "hierarchy": {
                ...
                "unknown_patient_files": unknown_patient_count,
                "warning": f"{unknown_patient_count} DICOM file(s) with missing/invalid PatientID excluded from patient count" if unknown_patient_count > 0 else None
            },
    ...
    def _list_image_types(self) -> str:
        ...
        if inventory.get('Images', {}).get('file_count', 0) > 0:
            image_types.append('Standard Images (JPG/PNG/etc) - verify if medical imaging')
        
        if not image_types:
            return "No imaging data detected"
        
        return "Image types found:\n" + "\n".join([f"  ✓ {img_type}" for img_type in sorted(set(image_types))])
    ...
    def _list_supporting_data(self) -> str:
        ...
        # Image analyses and segmentations
        rt_data = dicom.get('radiotherapy', {})
        if rt_data.get('has_rt_data'):
            rt_counts = rt_data.get('rt_structure_counts', {})
            rt_types = []
            if 'RTSTRUCT' in rt_counts:
                rt_types.append(f"Segmentations (RTSTRUCT: {rt_counts['RTSTRUCT']} files)")
            if 'RTDOSE' in rt_counts:
                rt_types.append(f"Dose (RTDOSE: {rt_counts['RTDOSE']} files)")
            if 'RTPLAN' in rt_counts:
                rt_types.append(f"Plans (RTPLAN: {rt_counts['RTPLAN']} files)")
            if rt_types:
                supporting_data.append("Image Analyses: " + ", ".join(rt_types))
        
        # Check for DICOM SEG (Segmentation) objects
        modalities = dicom.get('modalities_found', {})
        if 'SEG' in modalities:
            supporting_data.append(f"Image Analyses: Segmentations (SEG: {modalities['SEG']} files)")
        
        # Check for DICOM SR (Structured Reports) - clinical findings
        if 'SR' in modalities:
            supporting_data.append(f"Structured Reports: {modalities['SR']} files (clinical findings)")
        
        # Code/Scripts - note these might not be "supporting data" for TCIA
        # But include them as they might be relevant for reproducibility
        if inventory.get('Code_Scripts', {}).get('file_count', 0) > 0:
            supporting_data.append(
                f"Software/Source Code: {inventory['Code_Scripts']['file_count']} files "
                f"(may not be required by TCIA)"
            )
        
        # Documents - similar note
    ...
    def _describe_file_formats(self) -> str:
        ...
        # Medical data categories relevant for TCIA
        medical_categories = {
            'DICOM', 'Medical_Imaging', 'Images', 'Pathology_Slides', 
            'Tabular_Data', 'Text_Documents'
        }
        ...
        for category, data in inventory.items():
            if data['file_count'] > 0:
                ...
                desc = f"{category}: {exts} format ({data['file_count']} files, {data['total_size_gb']:.2f} GB)"
                
                if category in medical_categories:
                    format_descriptions.append(desc)
                elif category in supporting_categories:
                    format_descriptions.append(desc + " (supporting data)")
                else:
                    # Code, Archives, etc. - note as auxiliary
                    auxiliary_descriptions.append(desc)
        
        result = "\n".join(format_descriptions)
        
        if auxiliary_descriptions:
            result += "\n\nAuxiliary files (may not be required by TCIA):"
            result += "\n" + "\n".join([f"  - {desc}" for desc in auxiliary_descriptions])
    ...
    def _capture_print_output(self, func, *args, **kwargs):
        """Helper method to capture print output from a function."""
    ...
    def save_summary_to_file(self, output_path: str = None, tcia_form_only: bool = False):
        """Save all summary output to a text file."""
        if output_path is None:
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            output_path = f"tcia_dataset_summary_{timestamp}.txt"
        
        try:
            with open(output_path, 'w', encoding='utf-8') as f:
                if not tcia_form_only:
                    # Capture comprehensive summary
                    comprehensive_summary = self._capture_print_output(
                ...
                # Capture TCIA form answers
                tcia_form_answers = self._capture_print_output(
                    self.print_tcia_form_answers
                )
                f.write(tcia_form_answers)
            
            self._log(f"Summary saved to: {output_path}")
            return output_path
        except Exception as e:
            self._log(f"Error saving summary to file: {str(e)}")
            raise
...
Examples:
  ...
  python tcia_analyzer.py /path/to/dataset --quiet
    ...
    try:
        ...
        # Save summary text file - use same base name as JSON file
        base_name = json_path.replace('.json', '')
        summary_path = f"{base_name}_summary.txt"
        analyzer.save_summary_to_file(summary_path, tcia_form_only=args.tcia_form_only)
