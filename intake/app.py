...
# ============================================================
# Controlled vocabulary loader + search helpers
# ============================================================
@st.cache_data
def load_vocab_files():
    """Load controlled vocabulary Excel files once and cache them."""
    diag_df = pd.read_excel("primary_diagnosis_caDSR_14905532.xlsx")
    site_df  = pd.read_excel("primary_site_caDSR_14883047.xlsx")
    diag_terms = diag_df["Permissible Value"].dropna().astype(str).unique().tolist()
    site_terms = site_df["Permissible Value"].dropna().astype(str).unique().tolist()
    return diag_terms, site_terms, diag_df, site_df

NCIT_TERMS, UBERON_TERMS, NCIT_DF, UBERON_DF = load_vocab_files()

def unified_term_search(user_input: str):
    """Simple multi-token contains search; returns top 10 candidates."""
    if not user_input:
        return []
    tokens = [t.strip().lower() for t in re.split(r"[|/,;]+", user_input) if t.strip()]
    if not tokens:
        return []
    all_terms = NCIT_TERMS + UBERON_TERMS
    cand = [t for t in all_terms if any(tok in t.lower() for tok in tokens)]
    return sorted(set(cand))[:10]
...
def header():
    ...
    logo_light = (os.getenv("BRAND_LOGO_URL_LIGHT")
                  or os.getenv("BRAND_LOGO_URL")
                  or "Light_logo.png")
    logo_dark  = (os.getenv("BRAND_LOGO_URL_DARK")
                  or os.getenv("BRAND_LOGO_URL")
                  or "Dark_logo.png")
    logo_src = logo_dark if theme == "dark" else logo_light
...
def data_collection_block():
    ...
    # >>> NEW: Primary disease site or diagnosis (inline, with Search button)
    st.markdown("**Primary disease site or diagnosis \\***")
    c1, c2 = st.columns([5, 1])

    with c1:
        typed_val = st.text_input(
            "Type the disease / diagnosis / site name",
            key="unified_typed",
            placeholder="e.g., blood | bone tissue",
            help="Type keywords (use | / , ; to separate). Click Search to fetch matches."
    )

    with c2:
        search_clicked = st.form_submit_button(
            "Search", use_container_width=True, type="secondary", key="unified_search_btn"
        )

    # remember if user clicked search; only then show matches
    if search_clicked:
        st.session_state["unified_searched"] = True
    show_results = st.session_state.get("unified_searched", False) and bool(typed_val.strip())

    unified_term_final = typed_val.strip()
    if show_results:
        suggestions = unified_term_search(typed_val)
        picked = st.selectbox(
            "Matches from controlled vocabularies",
            ["(keep what I typed)"] + (suggestions or []),
            key="unified_pick",
        )
        if picked != "(keep what I typed)":
            unified_term_final = picked

        # details
        if unified_term_final and (unified_term_final in NCIT_TERMS or unified_term_final in UBERON_TERMS):
            with st.expander("Details for selected controlled term"):
                df_src = NCIT_DF if unified_term_final in NCIT_TERMS else UBERON_DF
                row = df_src[df_src["Permissible Value"] == unified_term_final].iloc[0].to_dict()
                for k, v in row.items():
                    st.markdown(f"**{k}:** {v}")

    # persist current selection for submit/validation
    st.session_state["unified_term_final"] = unified_term_final
    st.caption(f"Selected term: `{unified_term_final}`" if unified_term_final else "No term selected yet.")

    # ---- original following questions (kept as-is) ----
    ...
    return {
        ...
        "unified_term": st.session_state.get("unified_term_final", "").strip(),  # >>> NEW
...
def submit_page():
    ...
    with st.form("proposal_form", clear_on_submit=False):
        ...
        if proposal_type == "New Collection":
            details = data_collection_block()      # >>> CHANGED: now includes inline controlled-term block
        else:
    ...
    if proposal_type == "New Collection":
        must = [
            ...
            (details["unified_term"], "Primary disease site or diagnosis (controlled term) is required."),  # >>> NEW
